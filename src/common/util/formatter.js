...
export const formatTime = (value, format, hours12) => {
  if (value) {
    ...
    switch (format) {
      ...
      case 'minutes':
        return m.format(hours12 ? 'DD/MM/YYYY hh:mm A' : 'DD/MM/YYYY HH:mm');
      default:
        return m.format(hours12 ? 'DD/MM/YYYY hh:mm:ss A' : 'DD/MM/YYYY HH:mm:ss');
    }
  }
  return '';
};
...
export const formatOdometer = (value, unit, t) => `${Number((value / 1000).toFixed(2)).toLocaleString()} ${distanceUnitString(unit, t)}`;
...
export const customGetStatusColor = (status, position) => {
  if (!position) {
    switch (status) {
      case 'online':
        return 'success';
